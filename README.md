...
**상태**: 2026-08-19 기준 후임자 미정입니다. 자격증명 실 값은 채림이 임시 보관 중이니,
후임자 확정 시 채림을 통해 받으세요.

**⚠️ 가장 중요한 것 한 가지**: `hayoung.kim` 계정이 삭제될 예정입니다. 삭제 전 이관이
안 됐다면 검색 UI/Motional 자동발행이 멈춰있을 수 있어요 →
[인수인계_자격증명관리.md](./handOver/인수인계_자격증명관리.md)의 "hayoung.kim 계정
삭제 전 이관 체크리스트"(또는 이미 삭제됐다면 바로 아래 "복구 절차") 확인.

**하는 법**: 새 서버에 클론하지 말고, **strad35 서버(10.50.32.140)를 그대로 이어받아
접속**하세요 — Elasticsearch 데이터, 이미지 스토리지 마운트, systemd 서비스가 전부 그
서버에만 있어서 새로 클론해도 작동하지 않습니다.

1. **[인수인계_압축본.md](./handOver/인수인계_압축본.md) 읽기** (10분) — 전체 그림 파악
2. **서버 접속** — [인수인계_상세본.md](./handOver/인수인계_상세본.md) 0-1 섹션 순서대로
3. **`.env` 세팅** — 값은 채림에게 요청 ([인수인계_자격증명관리.md](./handOver/인수인계_자격증명관리.md) 참고)
4. **`python3 audit_pipeline.py` 실행** — "불일치/누락: 0개" 나오면 정상 세팅 완료
5. **막히면** → [인수인계_상세본.md](./handOver/인수인계_상세본.md)의 "트러블슈팅 노하우" 표에서 증상 검색
