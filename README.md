...
## 📍 현재 인수인계 상태 (읽는 시점에 따라 달라질 수 있음)

이 레포와 문서를 작성한 시점(2026-08-19) 기준, **다음 담당자가 아직 확정되지
않았습니다.** 작성자(김하영)는 인턴을 종료했고, 후임자 지정 관련 문의는 관리자에게
전달했으나 답변 대기 중인 상태에서 이 문서를 작성했습니다.

- 이 문서를 읽고 계신 분이 **공식적으로 이 프로젝트의 후임 담당자로 지정된 것이
  아니라면**, 임의로 서버 접속/설정 변경을 진행하기보다 먼저 관리자에게 담당자 지정
  여부를 확인해주세요.
- 자격증명(비밀번호 등) 실 값은 **채림이 임시로 보관 중**입니다. 후임자가 공식
  확정되면 채림을 통해 전달받으시면 됩니다. (자세한 내용은
  [인수인계_자격증명관리.md](./handOver/인수인계_자격증명관리.md) 참고)
- ⚠️ `hayoung.kim` 계정은 로그인 차단이 아니라 **실제 삭제 예정**입니다. 삭제 전 반드시
  [인수인계_자격증명관리.md](./handOver/인수인계_자격증명관리.md)의 "hayoung.kim 계정
  삭제 전 이관 체크리스트" 섹션을 먼저 확인해주세요 (안 하면 검색 UI와 Motional 자동
  발행이 즉시 중단됩니다).
