...
사용법은 이전과 동일:
    ...
    python3 vllm_query_parser_v6.py "야간에 소가 ego 차량 앞으로 지나가는 시나리오"
"""
...
def build_schema_with_meaning() -> str:
    ...
    # aaa1_context는 "필드명: 허용값 목록과 각 값의 뜻"을 명확히 분리해서 보여줌
    # (예전 방식은 "time:day : 낮 시간대..." 처럼 필드명+값을 통째로 보여줘서
    #  모델이 값으로 "time:day" 전체를 그대로 뱉는 버그가 있었음)
    context_field_order = ["time", "da_road_type", "weather"]
    ...
    for field_name in context_field_order:
        groups["aaa1_context"].append(f"- {field_name} (allowed values):")
        groups["aaa1_context"].extend(context_options[field_name])
...
Example 6 (context field's allowed values don't cover the concept — do NOT force the closest option)
User: "터널 안에서 오토바이 헤드램프가 보이는 상황"
Reasoning: "motorbike_headlamp" matches cleanly. But "터널"(tunnel) is not among da_road_type's
allowed values (public_road_highway/public_road_city/public_road_rural/parking_lot_outdoor/
parking_lot_indoor) — a tunnel is none of these, so do NOT force it into "public_road_highway" just
because tunnels are often on highways. Check lsd_tags too: reflector_tunnel_lowmid_light means
lights/reflections in the middle-lower part of a tunnel interior, which is a different concept
(interior lighting) from "being inside a tunnel" as a road-type context — it doesn't state road type
either, so still no exact match for the "tunnel" context itself. Put it in unmatched instead.
Output:
{{
  "logic": "AND",
  "conditions": [
    {{"group": "lsd_tags", "field": "motorbike_headlamp", "value": "present"}}
  ],
  "unmatched": ["터널이라는 도로 유형(맥락) - da_road_type 허용값 목록에 터널 옵션 없음"]
}}
...
- "value" for aaa1_context fields must be EXACTLY one of that field's own listed allowed values
  (day/dawn_evening/night for time; public_road_highway/public_road_city/public_road_rural/
  parking_lot_outdoor/parking_lot_indoor for da_road_type; clear/rainy/heavy_rainy/snow/fog for
  weather) — never the field name itself (e.g. never output "time:day", only "day"), and never a
  value outside this exact list. If the sentence describes a context that doesn't fit any listed
  value (e.g. a tunnel, which is none of the da_road_type options), that condition goes to unmatched
  instead of being forced onto the closest-sounding option.
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('사용법: python3 vllm_query_parser_v6.py "검색하고 싶은 문장"')
        sys.exit(1)
