# -*- coding: utf-8 -*-
"""
20단계 프로토타입: vLLM(Qwen) 쿼리 파서 v20
v19 대비 변경점 - B-1 hallucination(빈 context 임의 채우기) 대응:
  32문항 확대 재검증(v19 vs 원본 프롬프트+14B 비교실험 포함)에서, 문장이 특정 필드
  (time/da_road_type/weather, 또는 aaa1_tag의 필수 전제조건)를 언급하지 않았는데도
  모델이 "그럴듯한" 값을 임의로 채워 넣는 패턴이 30~40% 재현율로 확인됨(예: "신호등
  앞에서 선행 차량과 함께 정지" -> 문장에 없는 time:day를 추가; "비가 오지 않는 도로"
  -> "맑은"이 없는데도 weather:clear로 확정). 비교실험 결과 이 패턴은 프롬프트 압축이
  원인이 아니라 모델(Qwen3-14B-FP8 + thinking-mode-off) 자체 특성에 가까웠으나, 그렇다고
  손 놓을 문제는 아니므로 프롬프트 레벨에서 명시적으로 방어하기로 함.

  기존 Rule 1("컨텍스트 값을 그럴듯해 보인다고 추가하지 말 것")과 Rule 7("명시/암시되지
  않은 조건을 추론하지 말 것")이 이 문제를 부분적으로 다루고는 있었으나, 둘 다 다른
  주제(필드명 발명 금지 / 과잉 조건 추론 금지)의 부속절로만 존재해서 "명시 안 된 필드는
  기본값이 아니라 생략"이라는 원칙이 독립적으로 강조되지 않았음 — 특히 "문장의 다른
  필드는 확실하게 찾았으니 나머지도 자신있게 채워도 되겠지"라는 방향의 실패를 직접
  겨냥하는 문구가 없었음.

대응 (B-1만 대상, B-2는 이번 범위 밖):
  - Rule 20 신규 추가(번호 유지 원칙에 따라 기존 규칙 끝에 추가): context 필드와
    aaa1_tag의 필수 전제조건은 "명시 안 됨 = 완전히 생략"이 기본값이며, 문장의 다른
    필드가 확실히 매칭됐다고 해서 나머지 필드까지 추측해서 채우면 안 된다는 원칙을
    독립 규칙으로 명시.
  - Few-shot 예시 3개 추가 (이번에 실제로 실패했던 문장을 그대로 사용, 정답은 이미
    확인된 것): "신호등 앞에서 선행 차량과 함께 정지해 있는 상황"(time 생략),
    "안개 낀 도로에서 차선이 안 보이는 상황"(전제조건 미충족 필드는 unmapped 처리),
    "비가 오지 않는 도로"(부정 표현만으로 clear 확정 금지 - 기존 "비가 오지 않는
    맑은 날"/"비가 오지 않는 흐린 날" 대조쌍 바로 옆에 세 번째 대조 예시로 배치).

v18 대비 변경점 (v19에서 그대로 승계) - 시스템 프롬프트 토큰 압축 (8,278 -> 5,498 토큰):
...
20. Context fields (time, da_road_type, weather) and any AAA1/LSD tag's required sub-conditions default
    to "unspecified = omit entirely". NEVER fill in a plausible-sounding value for a field the sentence
    does not mention or clearly imply, even if the query feels "incomplete" without it. This applies
    even when OTHER fields in the same sentence ARE clearly matched — confidence in the fields you did
    find must NOT lead you to guess the ones you didn't. When in doubt whether something is implied vs
    merely plausible, omit it.
...
User: "신호등 앞에서 선행 차량과 함께 정지해 있는 상황"
Reasoning: "선행 차량"과 "정지"는 명시됐으므로 stationary_at_traffic_light_with_lead로 매칭.
문장에 낮/밤에 대한 언급이나 암시가 전혀 없음 — time을 day로 추측해 채우면 안 됨, 그냥 생략.
Output: {{"query":{{"field":"motional_scenario","value":"stationary_at_traffic_light_with_lead"}},"unmapped_terms":[]}}

User: "안개 낀 도로에서 차선이 안 보이는 상황"
Reasoning: "안개"->weather=fog. "차선이 안 보임"은 invisible_lanes_city_3_5_lanes_many_vehicles_ahead와
비슷해 보이지만, 이 필드는 "도심 3~5차선+앞차 많음"이라는 구체적 전제조건이 함께 있어야 매칭됨 —
문장에 도로 유형이나 차량 밀집도 언급이 전혀 없으므로 전제조건 미충족 필드는 매칭하지 말고 unmapped 처리.
Output: {{"query":{{"field":"weather","value":"fog"}},"unmapped_terms":["차선이 안 보이는 상황 - no_lane_mark_road는 차선 표시 자체가 없는 도로를 의미, invisible_lanes_city_3_5_lanes_many_vehicles_ahead는 도심 3~5차선+차량밀집 전제조건이 문장에 없어 매칭 보류"]}}
...
User: "비가 오지 않는 도로"
Reasoning: "비가 오지 않는다"는 것만으로는 맑음/눈/안개 등 여러 날씨가 가능함 — "맑은"처럼 명시적으로
clear를 가리키는 단어가 있을 때만(위 예시) weather=clear로 매칭해야 함. 이 문장은 부정 표현만 있고
긍정 지목이 없으므로 weather 필드 자체를 생략.
Output: {{"query":null,"unmapped_terms":["비가 오지 않는 도로 - 특정 날씨(맑음/눈/안개 등)를 확정할 근거 없음, 부정 표현만으로는 weather 값을 특정할 수 없음"]}}
