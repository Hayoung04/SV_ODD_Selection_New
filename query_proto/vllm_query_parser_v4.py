# -*- coding: utf-8 -*-
"""
5단계 프로토타입: vLLM(Qwen) 쿼리 파서 v4
v3 대비 변경점:
  1) 필드 이름만 아니라 Meaning(설명)까지 스키마에 포함 -> 모델이 필드 의미를 정확히 알고 매칭
  2) 매칭 안 되는 조건은 "unmatched"에 명시적으로 적게 함 -> 억지 매핑 억제 + 스키마 갭 자동 수집
  3) 부정 예시(매칭 실패 케이스) 추가

사용법은 이전과 동일:
    export VLLM_BASE_URL="http://127.0.0.1:8001/v1"
    export VLLM_MODEL_NAME="Qwen/Qwen3-VL-8B-Instruct"
    export VLLM_API_KEY="dummy"
    python3 vllm_query_parser_v4.py "야간에 소가 ego 차량 앞으로 지나가는 시나리오"
"""
...
- Match a condition to a field ONLY if the field's stated meaning genuinely covers that condition.
  Superficial keyword overlap (e.g. both mention "crossing") is NOT enough if the entity or context
  differs (e.g. animal vs pedestrian vs vehicle).
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('사용법: python3 vllm_query_parser_v4.py "검색하고 싶은 문장"')
        sys.exit(1)
