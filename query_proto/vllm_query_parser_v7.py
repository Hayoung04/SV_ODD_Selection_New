# -*- coding: utf-8 -*-
"""
7단계 프로토타입: vLLM(Qwen) 쿼리 파서 v7
- 출력 구조/핵심 규칙: 시은의 프롬프트를 채택 (중첩 Boolean all/any, absent 처리 규칙, 구체성 유지 규칙)
- taxonomy 내용: 우리가 검증한 상세 필드 Meaning(field_descriptions.py) 사용
- 추가 예시: 우리가 실제로 발견한 실패 케이스(터널 억지매핑, 실내주차장 급가속, 동물+동작 조합)를
  taxonomy 뒤에 few-shot으로 덧붙임

사용법:
    export VLLM_BASE_URL="http://127.0.0.1:8001/v1"
    export VLLM_MODEL_NAME="Qwen/Qwen3-VL-8B-Instruct"
    export VLLM_API_KEY="dummy"
    python3 vllm_query_parser_v7.py "야간에 소가 ego 차량 앞으로 지나가는 시나리오"
"""
...
8. Preserve specificity: e.g. "left lane change" -> changing_lane_to_left (not the generic changing_lane);
   generic "lane change" -> changing_lane.
9. The input may be Korean or English.
10. Return JSON only, exactly with keys: query, unmapped_terms.
11. `query` may be one atomic condition, one Boolean group, or null if nothing is searchable.
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('사용법: python3 vllm_query_parser_v7.py "검색하고 싶은 문장"')
        sys.exit(1)
